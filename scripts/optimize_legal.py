"""CLI: optimize LEGAL specialist system prompt with DSPy (COPRO).

Writes a sidecar file under artifacts/prompts/ — does not overwrite legal.py.
"""
...
from arcs.optimization.dspy_legal import (
    DEFAULT_OUTPUT,
    DEFAULT_QUEUE,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Optimize the LEGAL specialist system prompt with DSPy COPRO "
            "using specialist_queue.jsonl LEGAL failures. Writes a sidecar "
            "prompt file for human review — does not modify legal.py."
        ),
    )
    parser.add_argument(
        "--queue",
    ...
    parser.add_argument(
        ...
        default=20,
        metavar="N",
        help="Max LEGAL examples to load from the queue (default: 20)",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
