...
# ── 1. Load data ──
train_df = pd.read_csv(config.ROUTER_DATA_DIR / "router_train.csv")
test_df = pd.read_csv(config.ROUTER_DATA_DIR / "router_test.csv")
...
trainer.save_model(str(config.ROUTER_MODEL_DIR))
print(f"Model saved to {config.ROUTER_MODEL_DIR}")
