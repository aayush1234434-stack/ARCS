...
def configure_lm(*, model: str | None = None) -> Any:
    ...
    if not api_key:
        raise RuntimeError("GROQ_API_KEY is required for medical prompt optimization")
...
def save_optimized_prompt(text: str, output_path: Path) -> Path:
    ...
    header = (
        "# Optimized MEDICAL specialist system prompt (DSPy COPRO)\n"
        "# Review this file, then manually replace SYSTEM_PROMPT in\n"
        "# arcs/pipelines/specialists/medical.py if it looks better.\n"
