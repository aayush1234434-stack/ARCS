...
completed all 40 sealed rows. The full ARCS condition stopped after 20 rows
(19 completed plus one quota error) when Groq's 200,000-token daily limit was
reached. The matched blinded pilot packet therefore uses the six rows for which
all three systems produced answers; see the ignored local artifact at
`artifacts/human-eval/sealed-v1-gpt20b-common6/`.
